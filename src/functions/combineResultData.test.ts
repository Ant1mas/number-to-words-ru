...
test('test', () => {
  expect(combineResultData(['+', '1501', '.', '22'])).toBe('Одна тысяча пятьсот один рубль 22 копейки');
  expect(combineResultData(['+', '1501', '.', ''], {
    currency: {
...
test('Знак минус', () => {
  expect(combineResultData(['-', '1501', '.', '22'])).toBe('Минус одна тысяча пятьсот один рубль 22 копейки');
  expect(combineResultData(['-', '1501', '.', '22'], {
...
test('Разные варианты currency', () => {
  ...
  expect(combineResultData(['-', '1501', '.', '224'], {
    currency: 'number'
  ...
  expect(combineResultData(['-', '1501', '/', '23'], {
    currency: 'number'
  })).toBe('Минус одна тысяча пятьсот одна 23');
  expect(combineResultData(['-', '1501', '.', ''], {
    currency: 'number'
  })).toBe('Минус одна тысяча пятьсот одна целая 0 десятых');
  expect(combineResultData(['-', '1501', '.', '000000'], {
    currency: 'number'
  })).toBe('Минус одна тысяча пятьсот одна целая 0 десятых');
  expect(combineResultData(['-', '1501', '.', '020000'], {
    currency: 'number',
...
test('Округление', () => {
  ...
  expect(combineResultData(['+', '1501', '.', '015'], {
    currency: 'number',
    roundNumber: -1,
...
test('Разделитель - дробная черта', () => {
  expect(combineResultData(['+', '1501', '/', '122'])).toBe('Одна тысяча пятьсот одна 122 рубля');
  expect(combineResultData(['+', '1501', '/', '122'], {
    convertNumbertToWords: {
      fractional: true,
    }
