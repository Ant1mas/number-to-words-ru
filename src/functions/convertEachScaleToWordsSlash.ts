...
import {slashNumberUnitPrefixes} from 'units/slashNumberUnitPrefixes';
import {
  fractionalUnitsBases,
  fractionalUnitEndings,
} from 'units/fractionalCurrencyNumber';
import unitNames from 'units/unitNames';
import ordinalNumbersDeclensions from 'units/ordinalNumbers/ordinalNumbers';
import convertEachScaleToWords from 'functions/convertEachScaleToWords';
import removeEmptyScalesBeforeNumber from 'functions/removeEmptyScalesBeforeNumber';
 ...
 * @param {number} unitNameForm - Род валюты целой части числа ([мужской, средний, женский]).
...
const convertEachScaleToWordsSlash = (
  ...
  const lastScaleWihNumber = [...updatedNumberScalesArr].reverse().findIndex((scale) => scale !== '000');
  // Индекс последнего класса в массиве.
  const lastScaleWihNumberIndex = updatedNumberScalesArr.length - lastScaleWihNumber - 1;
  ...
  if (updatedNumberScalesArr.length - 1 - lastScaleWihNumber > 0) {
    ...
    const numberScalesArrForCommonConvert = updatedNumberScalesArr.map((scale, index) => {
  ...
  if (lastScaleWihNumber >= 1) {
    ...
    const scaleToConvertArr: number[] = updatedNumberScalesArr[lastScaleWihNumberIndex].split('').map((digit) => parseInt(digit));
    ...
    const tens = (scaleToConvertArr[1] === 1) ? slashNumberUnitPrefixes.tenToNineteen[scaleToConvertArr[2]] : slashNumberUnitPrefixes.tens[scaleToConvertArr[1]];
    // Конвертировать единицы ("пяти")
    let digits = (scaleToConvertArr[1] === 1) ? '' : slashNumberUnitPrefixes.digit[scaleToConvertArr[2]];
    ...
    const unitNameBase = (lastScaleWihNumber <= fractionalUnitsBases.length) ? fractionalUnitsBases[lastScaleWihNumber - 1] : unitNames[lastScaleWihNumber - 2];
    // Получить окончание названия класса числа с правильным падежом ("ная", "ной", "ных" и т.д.)
    const unitNameEnding = selectDataByDeclension(fractionalUnitEndings, declension, (unitNameForm === 0) ? false : true);
    // Добавить текст к общему результату
    convertedResult += `${hundreds}${tens}${digits}${unitNameBase}${unitNameEnding}` + ' ';
  }
  ...
  if (lastScaleWihNumber === 0) {
    ...
    const lastDigitInScale = [...scaleToConvertArr].reverse().findIndex((digit) => digit !== 0);
    // Получить индекс последнего разряда в текущем классе
    const lastDigitInScaleIndex = scaleToConvertArr.length - lastDigitInScale - 1;
    /* Получить класс без последнего разряда для конвертирования как обычного числа
    и если после этого в разраде десяток остается "1", то эту "1" тоже убрать,
    чтобы не отконвертировалось как 10-19. */
    const scaleForCommonConvert = [
      scaleToConvertArr.map((digit, index) => {
